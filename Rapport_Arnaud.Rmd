---
title: "Probabilités Appliquées - Projet simulation de file d'attente"
...
Les requêtes arrivent aux temps A1,A2,..,An en suivant la une loi exponentielle de paramètre $lambda. Afin de gérer les niveaux de priorités (similaire à ce que ferai un système de QoS), les requêtes entrantes sont stockées dans une des trois sous-file de priorité avec le choix de la file réalisé grâce à une variable aléatoire suivant une loi uniforme.
...
Le taux de perte est donc élevé puisqu'il monte au delà de 40% pour un ratio $\lambda/\mu$=10.
...
Pour une queue de 10 éléments et un ratio de $\lambda/\mu$ de 12, il suffit de 4 serveurs pour arriver à un nombre nul ou quasi nul de paquets perdus.